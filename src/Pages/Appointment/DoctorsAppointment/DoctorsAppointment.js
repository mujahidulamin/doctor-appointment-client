...
const DoctorsAppointment = ({ selectedDate }) => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://doctor-appointment-server-university.vercel.app/doctors?search=${search}&date=${date}`
      );
  ...
  return (
    <section className="pb-10">
      <div className=" mb-10 ">
        <div>
          <p className="text-center font-semibold lg:text-2xl md:text-2xl mb-5 text-white">
            Available Appointments on{" "}
            <span className="text-[#e12986]">{format(selectedDate, "PP")}</span>
          </p>
        </div>
